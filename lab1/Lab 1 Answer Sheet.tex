...
\end{center}
  ...
  \framebox{
    \begin{minipage}{5.5in}
      \vspace{0.2cm}
      \textbf{The Problem with MLE:} Maximum Likelihood Estimation relies solely on observed counts. If a word $w$ never appears in the training data for a specific class $y$, the estimated probability is zero: $\hat{P}(w|y) = 0$.

      \textbf{The Consequence:} The Naive Bayes classifier calculates the likelihood of an email by multiplying the probabilities of all its words:
      ...
      \[
      \hat{\theta}_{d} = \frac{N_{d} + 1}{N_{total} + D} > 0
      \]
      \vspace{0.1cm}
    \end{minipage}
  }
